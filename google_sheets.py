...
# 🔐 Load Google Sheet credentials from GitHub secret
def get_google_credentials():
    raw_json = os.environ.get("NEW")  # NEW is your GitHub secret
    if not raw_json:
        raise Exception("Environment variable 'NEW' is not set.")
    json_dict = json.loads(raw_json)
    creds = ServiceAccountCredentials.from_json_keyfile_dict(json_dict, [
